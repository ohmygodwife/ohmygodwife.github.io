...
###### [fuelvm](https://ctf-wiki.github.io/ctf-wiki/reverse/vm/vm-zh/)
...
File->Script file选择[IDAGolangHelper](https://github.com/sibears/IDAGolangHelper)的go_entry.py脚本，点击Rename functions，还原符号。
...
对于此种exe，使用[pyinstallerextractor](https://sourceforge.net/projects/pyinstallerextractor/)提取，**切记：需要使用和打包时一致的python版本！**

```powershell
>python3 D:\ctf\tools\re\PyinstallerExtractor\pyinstxtractor.py AnhengRe.exe
```

在抽取出来的文件中找到同名无后缀文件AnhengRe，加上对应版本12字节的pyc头，增加后缀pyc
...
```
./pycdc [PATH TO PYC FILE] #PYC Decompiler
./pycdas [PATH TO PYC FILE] #PYC Disassembler
```

如果[opcode前面有混淆代码](https://www.52pojie.cn/thread-912103-1-1.html)（类似于c的花指令），则需要先将去掉，修改`co_code`长度即可正常反编译。
