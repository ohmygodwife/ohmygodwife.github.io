...
该函数注册的函数会在程序结束的时候进行逆序的 运行，逆序的序就是函数注册的顺序。sample：[你主函数保真吗](https://xz.aliyun.com/t/15448)（羊城杯2024）
...
[obpo-plugin](https://github.com/obpo-project/obpo-plugin)
...
```
java -jar D:\ctf\tools\android\apktool_2.12.1.jar d five.apk #depackage
java -jar D:\ctf\tools\android\apktool_2.12.1.jar b five -o new.apk #package
java -jar D:\ctf\tools\android\uber-apk-signer-1.3.0.jar -a new.apk -o ./out #sign with built-in debug keystore. 
...
使用[de4dot](https://github.com/ViRb3/de4dot-cex)去混淆：`D:\ctf\tools\re\de4dot-cex\de4dot.exe -f sample.bin`
...
```powershell
D:\ctf\tools\re\pyinstxtractor-ng\pyinstxtractor-ng.exe build.exe
...
- https://pylingual.io/（支持Python3.12）
...
```
/mnt/hgfs/ctf/tools/re/pycdc/pycdc [PATH TO PYC FILE] #PYC Decompiler
/mnt/hgfs/ctf/tools/re/pycdc/pycdas [PATH TO PYC FILE] #PYC Disassembler
报Unsupported opcode: JUMP_BACKWARD错误时，更新pycdc代码，实在不行，修改：ASTree.cpp，在2478行default处加上break，再重新编译跳过该报错。https://idafchev.github.io/blog/Decompile_python/
```
