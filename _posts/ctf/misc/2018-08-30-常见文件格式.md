...
3. 由于flag前后的变量值长度均为0x40+1，根据对齐方式0x20，每项长度实际为0x60，而flag长度不足0x40，因此根据对齐方式0x20时，只会占据0x40。因此，从data节头开始，以二分方式，查找0x60的倍数+0x20，如果遇到十六进制字符，表示flag在后半段，如果遇到0x00，表示flag在前半段，直至遇到非十六进制字符。
