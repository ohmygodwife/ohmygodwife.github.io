...
#### (p-1)和(q-1)有一个大的公因数，[Further Attacks on Server-Aided RSA Cryptosystems](https://hasegawaazusa.github.io/common-prime-rsa.html,https://xz.aliyun.com/t/6703)
...
e*d=k*\phi(N)+1\\
得到：k*\phi(N)+1=0 \mod e\\
因为\phi(N)=(p-1)*(q-1)=N-t+1，其中t为包含p，q的多项式（可扩展到多个因子），代入上式得：\\
k*(N-t+1)+1=0 \mod e\\
f(x,y) = x(N+1-y)+1=0 \mod e\\
求得此方程的根，即可得到\phi(N)
$$

###### [CF](https://www.yuque.com/yuqueyonghu30d1fk/gd2y5h/ksfsfw8yf1u2xhhq)（dasctf-202406）

###### [bd](https://www.cnblogs.com/mumuhhh/p/17789591.html)（领航杯-2023）：已知p，q高位
...
###### 已知：hint=pow((11*p+83), q, n)，crypto-25（20241119lanzhou）

$$
(11*p+83)^q=kn+h\\
(11*p+83)^{q*p}=(kn+h)^p\\
两边对p取模：83^n=h^p\mod p=h
$$

p = GCD(pow(83, n, n) - hint, n)
...
已知N和phiN，[d泄露攻击](https://ctf-wiki.org/crypto/asymmetric/rsa/d_attacks/rsa_d_attack/)，[Equivalence of Factoring and Exposing the Private Key](https://link.springer.com/content/pdf/10.1007/3-540-36492-7_25.pdf)

#### p，q字节仅有b'\x00'和b'\x01'，randfunc=random.getrandbits(1)，回溯剪枝

###### [ez_rsa](https://www.yuque.com/yuqueyonghu30d1fk/gd2y5h/ksfsfw8yf1u2xhhq)（dasctf-202406）

根据[Crypto.Util.Number](https://github.com/pycrypto/pycrypto/blob/master/lib/Crypto/Util/number.py)，getRandomNBitInteger函数保证随机数最高位必定为1。因此从低位开始，回溯生成p，q进行测试。

#### p = r + (k * q - r) % T，[RSA后门](https://arxiv.org/pdf/2201.13153)

[ez_rsa](https://www.yuque.com/yuqueyonghu30d1fk/gd2y5h/ksfsfw8yf1u2xhhq)（dasctf-202406）

k小，得到所有可能的(q mod T, p mod T)，C<n/T**2小，穷举C，即可还原p，q。
...
#### 两个pow(p+num1,e1,n2)，e不大，pow(q+q1,2024,n)

[ez_RSA](https://tangcuxiaojikuai.xyz/post/b1a746d0.html)（dasctf-202410），对多项式求GCD，求得p。对q取模，q=gcd(leak1-(leak2-2024)^2024, n)
