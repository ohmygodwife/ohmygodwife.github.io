...
[延时函数](https://www.cnblogs.com/c1e4r/articles/9060525.html)：`sleep()`，`BENCHMARK(100000,SHA1('true'))`，通过较大的表做笛卡尔积`select count(*) from information_schema.tables A,information_schema.tables B, information_schema.tables C `

条件语句：`CASE WHEN 1=1 THEN true else false ENd `，`IF(1=1, true, false)`，`IFNULL()`，`NULLIF()`
